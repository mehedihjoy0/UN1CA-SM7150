...
ZIP_FILE_NAME="UN1CA-Lite_${ROM_VERSION}_$(date +%Y%m%d)_${TARGET_CODENAME}${ZIP_FILE_SUFFIX}"
while [ -f "$OUT_DIR/$ZIP_FILE_NAME" ]; do
    INCREMENTAL=$((INCREMENTAL + 1))
    ZIP_FILE_NAME="UN1CA-Lite_${ROM_VERSION}_$(date +%Y%m%d)-${INCREMENTAL}_${TARGET_CODENAME}${ZIP_FILE_SUFFIX}"
done
